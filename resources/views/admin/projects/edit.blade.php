...
@push('styles')
<style>
    /* Animasi dan efek premium - konsisten dengan halaman index */
    .shadow-3d {
...
<div class="space-y-6 p-4 md:p-6">
    ...
    <div class="bg-white rounded-2xl p-6 shadow-3d border border-brown-dark/10 flex flex-wrap justify-between items-center gap-4 premium-card">
        <div class="flex items-center gap-4">
            ...
            <div>
                <h1 class="text-2xl font-bold text-brown-dark">Edit Project</h1>
                <p class="text-brown-medium text-sm flex items-center gap-1 mt-1">
                    <i class="fas fa-home"></i>
                    <span>Dashboard / Project / Edit Project</span>
                </p>
            </div>
        </div>
    ...
    <div class="bg-white rounded-2xl shadow-3d border border-brown-dark/10 overflow-hidden premium-card">
        <form action="{{ route('admin.projects.update', $project->id) }}" method="POST" enctype="multipart/form-data" id="projectForm" class="p-8 space-y-8">
            @csrf
            @method('PUT')

            <!-- Informasi Sistem (Readonly) -->
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4 info-readonly">
                ...
                <div class="flex items-center gap-2 text-brown-dark">
                    ...
                    <span>{{ $project->created_at ? $project->created_at->format('d M Y H:i') : '-' }}</span>
                </div>
                <div class="flex items-center gap-2 text-brown-dark">
                    <i class="fas fa-edit"></i>
                    <span class="font-medium">Diperbarui:</span>
                    <span>{{ $project->updated_at ? $project->updated_at->format('d M Y H:i') : '-' }}</span>
                </div>
            </div>
            ...
            <div class="space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <input type="text" name="client_name" value="{{ old('client_name', $project->client) }}"
                ...
                <div>
                    ...
                    <textarea name="short_description" rows="3"
                              class="w-full px-5 py-3 border border-brown-dark/20 rounded-xl focus:ring-2 focus:ring-brown-dark/20 focus:border-brown-dark transition">{{ old('short_description', $project->short_description) }}</textarea>
                ...
                <div>
                    ...
                    <textarea name="description" rows="6"
                              ...
                              placeholder="Jelaskan detail project secara lengkap..." required>{{ old('description', $project->description) }}</textarea>
                    @error('description')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
            ...
            <div class="space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <input type="number" name="team_size" value="{{ old('team_size', $project->team_size) }}"
                               class="w-full px-5 py-3 border border-brown-dark/20 rounded-xl focus:ring-2 focus:ring-brown-dark/20 focus:border-brown-dark transition"
            ...
            <div class="space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <textarea name="scope" rows="3"
                                  class="w-full px-5 py-3 border border-brown-dark/20 rounded-xl focus:ring-2 focus:ring-brown-dark/20 focus:border-brown-dark transition">{{ old('scope', $project->scope) }}</textarea>
                    </div>
                    ...
                    <div>
                        ...
                        <input type="text" name="sub_services" value="{{ old('sub_services', is_array($project->sub_services) ? implode(', ', $project->sub_services) : $project->sub_services) }}"
                               class="w-full px-5 py-3 border border-brown-dark/20 rounded-xl focus:ring-2 focus:ring-brown-dark/20 focus:border-brown-dark transition"
                    ...
                    <div>
                        ...
                        <input type="text" name="certifications" value="{{ old('certifications', is_array($project->certifications) ? implode(', ', $project->certifications) : $project->certifications) }}"
                               class="w-full px-5 py-3 border border-brown-dark/20 rounded-xl focus:ring-2 focus:ring-brown-dark/20 focus:border-brown-dark transition"
                    ...
                    <div>
                        ...
                        <input type="text" name="tags" value="{{ old('tags', is_array($project->tags) ? implode(', ', $project->tags) : $project->tags) }}"
                               class="w-full px-5 py-3 border border-brown-dark/20 rounded-xl focus:ring-2 focus:ring-brown-dark/20 focus:border-brown-dark transition"
                ...
                <div>
                    ...
                    <textarea name="testimonial" rows="4"
                              class="w-full px-5 py-3 border border-brown-dark/20 rounded-xl focus:ring-2 focus:ring-brown-dark/20 focus:border-brown-dark transition">{{ old('testimonial', $project->testimonial) }}</textarea>
                ...
                <div>
                    ...
                    class="w-full px-5 py-3 border border-brown-dark/20 rounded-xl focus:ring-2 focus:ring-brown-dark/20 focus:border-brown-dark transition">
                    {{ old('details', is_array($project->details) ? implode(', ', $project->details) : $project->details) }}
                </textarea>
                </div>
            </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <select name="status" class="w-full px-5 py-3 border border-brown-dark/20 rounded-xl focus:ring-2 focus:ring-brown-dark/20 focus:border-brown-dark transition">
                            <option value="ongoing" {{ old('status', $project->status) == 'ongoing' ? 'selected' : '' }}>Ongoing - Berjalan</option>
                            <option value="completed" {{ old('status', $project->status) == 'completed' ? 'selected' : '' }}>Completed - Selesai</option>
                            <option value="pending" {{ old('status', $project->status) == 'pending' ? 'selected' : '' }}>Pending - Ditunda</option>
                        </select>
                    </div>
                    ...
                    <div>
                        ...
                        @if($project->image)
                        <div class="mb-4">
                            <p class="text-sm text-brown-medium mb-2">Gambar Saat Ini:</p>
                            <div class="relative inline-block">
                                <img src="{{ asset('storage/'.$project->image) }}" alt="Preview" class="h-32 rounded-lg border border-brown-dark/10 shadow-md">
                            </div>
                        </div>
                        @endif
                        ...
                        <div class="upload-area border-2 border-dashed border-brown-dark/20 rounded-xl p-6 text-center hover:border-brown-dark transition cursor-pointer" onclick="document.getElementById('imageInput').click()">
                            ...
                            <p class="text-brown-dark font-medium" id="uploadText">Klik untuk upload (biarkan kosong jika tidak ingin mengubah)</p>
...
<script>
    ...
    function removeImage() {
        ...
        document.getElementById('uploadText').textContent = 'Klik untuk upload (biarkan kosong jika tidak ingin mengubah)';
    }
